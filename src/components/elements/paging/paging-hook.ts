...
export const usePaging = <T>(
   ...
   const handlePreviousPage = () => {
      if (pageRequest && pageRequest.page > 0) handlePageChange(pageRequest.page - 1);
   };

   const handleNextPage = () => {
      if (pageRequest && pages && pageRequest.page < pages.length - 1) handlePageChange(pageRequest.page + 1);
   };
