...
const Paging: React.FC<Props> = ({ pages = [], currentPage, previousPage, nextPage, gotoPage }) => {
   ...
   const showPageNumber = (page: Page) => {
      ...
      if (isFirstPage(page) || isLastPage(page)) {
         ...
         return (
            pages.length <= hideThreshold ||
            (page.index >= currentPage.index - 1 && page.index <= currentPage.index + 1)
         );
      }
   };
         ...
         <div>
            ...
            <div className="panel no-m-bottom">
               <nav className="pagination is-centered" role="navigation" aria-label="pagination">
                  <a
                     className={'pagination-previous button' + (isFirstPage(currentPage) ? ' is-static' : '')}
                  ...
                  <a
                     className={'pagination-next button' + (isLastPage(currentPage) ? ' is-static' : '')}
                     onClick={() => nextPage()}
                  >
                     Next page
                  </a>
                  ...
                  <ul className="pagination-list">
                     <li>
                        <a
                           className={'pagination-link ' + (isFirstPage(currentPage) ? ' is-current' : '')}
                           onClick={() => gotoPage(0)}
                        ...
                        if (showPageNumber(page)) {
                           return (
                              <li key={'p_' + page.index}>
                                 <a
                                    className={'pagination-link' + (isCurrentPage(page) ? ' is-current' : '')}
                     ...
                     {showLastPage() && (
                        <li>
                           <a
                              className={'pagination-link ' + (isLastPage(currentPage) ? ' is-current' : '')}
                              onClick={() => gotoPage(pages.length - 1)}
                              aria-label={'Goto page ' + pages.length}
                           >
                              {pages.length}
                           </a>
                        </li>
